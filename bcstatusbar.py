...
class BCStatusBar():
    ...
    def render_center(self,width):
        ...
        if time_of_day >= self._bcgi.SLEEP:
            day_time = (self._bcgi.DAY_FULLDAY-(self._bcgi.estimated_day_time()%self._bcgi.DAY_FULLDAY))/20
        else:
            day_time = (self._bcgi.DAY_SLEEP-(self._bcgi.estimated_day_time()%self._bcgi.DAY_FULLDAY))/20
        day_time_string = '{: 2}:{:02} '.format(floor(abs(day_time)/60),floor(abs(day_time)%60))
        self._status_bar_window.addstr(0, (width//2)-(len(day_time_string)//2),day_time_string,curses.color_pair(BCHudConstants.COLOR_STATUS_BAR))
#        self._status_bar_window.addstr(0, (width//2)-(len(day_time_string)//2),day_time_string,curses.color_pair(statusbar_esttimeofday))
