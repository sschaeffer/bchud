...
def main(stdscr, minecraftdir, servername, worldname):
    ...
    testpad = curses.newpad(100,width)
    ...
    while (key != ord('q')):
        ...
        if (activewindow==1):
            statusbarwin.addstr("Status Bar Win") 
            statusbarwin.noutrefresh()
            statpanel.hide()
            statusbarpanel.show()
        elif (activewindow==2):
            timerwindow.addstr("Timer Bar Win") 
            timerwindow.noutrefresh()
            statpanel.show()
            statusbarpanel.hide()
        elif (activewindow==3):
            for i in range(99):
                testpad.addstr(i,0,f"{i}: TEST PAD") 
            timerwindow.noutrefresh()
            testpad.refresh(0,0,1,0,height-2,width)
            statpanel.hide()
            statusbarpanel.hide()
        else:
            statpanel.hide()
